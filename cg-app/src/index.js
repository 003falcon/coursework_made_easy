...
function CGPACalculator() {
  ...
  const [coursesPerCredit, setCoursesPerCredit] = useState({4:0,3:0,2:0,1:0});
  ...
  const handleCoursesChange = (c, val) => {
    ...
    setGrades((prev) => ({
      ...prev,
      [c]: (prev[c] || []).slice(0, n).concat(Array(Math.max(0, n - (prev[c]?.length || 0))).fill(8)),
    }));
  };
  ...
  const handleCompute = () => {
    ...
    if (curSemCreditsList.length === 0) {
      setResult(<span style={{ color: "red" }}>Add at least one current semester course!</span>);
      return;
    }

    const totalCreditsAll = totalCredits + curSemCreditsList.reduce((a,b)=>a+b,0);
    const targetSum = desiredCG * totalCreditsAll;
    const neededSum = targetSum - weightedSum;

    // If already achieved
    if (neededSum <= curSemCreditsList.length * GRADE_MAX * Math.max(...curSemCreditsList)) {
      ...
      dfs(0, 0, []);
      if (count === 0) {
        setResult(<span style={{ color: "red" }}>Sorry, target CG cannot be achieved with the current configuration.</span>);
        return;
      }
      setResult(
        <div>
          ...
          {scenarios.map((gradesArr, si) => (
            <div key={si} style={{ margin: "6px 0", background:"#f1f7ff", padding: "5px 10px", borderRadius:4 }}>
              {gradesArr.map((g, gi) => (
                <span key={gi}>Course {gi+1} ({curSemCreditsList[gi]} credit): <b>{g}</b> {gi<gradesArr.length-1? ' | ':''}</span>
              ))}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="cgpa-root">
      <h2 style={{textAlign:"center",marginBottom:14}}>CGPA Target Calculator</h2>
      ...
      <div className="cgpa-section">
        <div style={{marginBottom:8}}><b>Completed Courses (before this semester):</b></div>
        <div style={{marginBottom:8}}>
        ...
        <div>
          ...
          />
          <button className="new-credit-add-btn" type="button" onClick={handleAddCredit}>Add Credit Type</button>
        </div>
      </div>

      {/* Step 3: Grades */}
      <div className="cgpa-section">
        <div style={{marginBottom:5}}><b>Enter Grades for Completed Courses</b></div>
        <div>
          {creditInputs.map(
            (c) =>
      ...
      {/* Step 4: Current Semester Info */}
      <div className="cgpa-section">
        <div style={{marginBottom:5}}><b>Add Current Semester Courses</b></div>
        <div>
          <input
            className="cgpa-input"
            type="number"
          ...
          />
          <button className="cgpa-add-btn" type="button" onClick={handleAddCurSemCourse}>Add</button>
        </div>
            ...
            <tbody>
              {currentSemCourses.map((row, idx) => (
                <tr key={idx}>
                  ...
                  <td>{row.credit}</td>
                  <td>
                    <button style={{background:"#f33",color:"#fff",border:"none",borderRadius:4,padding:"2px 8px"}} onClick={()=>handleRemoveCurSemRow(idx)}>X</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {/* Step 5: Compute */}
      <button className="cgpa-calc-btn" type="button" onClick={handleCompute}>Compute Scenarios</button>
      <div className="cgpa-result">{result}</div>
      <div className="cgpa-footer">
        <hr />
        Built with React. <br/>
        <b>Note:</b> For large number of courses, only a sample of possible scenarios will be shown.
      </div>
    </div>
  );
}
